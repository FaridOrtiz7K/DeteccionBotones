...
class ImageSearchController:
    ...
    def handle_b4_special_behavior(self, imagen, clicks, confianza):
        ...
        if self.model.current_lote == 1:
            ...
            pyautogui.press('alt')
            time.sleep(2)
            for i in range(1, self.model.current_lote):
                pyautogui.press('arrow_down')
                time.sleep(1)
    ...
    def run_lotes(self):
        ...
        while current_lote <= lote_final and self.model.is_running:
            ...
            if self.model.distrito == "NINUGO":
                self.nombre_archivo = f"LT "
            else:
                self.nombre_archivo = f"{self.model.distrito}_LT "
            
            self.view.log_message(f"Procesando archivo: {self.nombre_archivo} {current_lote}.kml")
